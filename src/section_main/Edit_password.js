...
/*-------------SYSTEM MESSAGE----------------------------------------*/
/*---------------------------------------------------------------*/
const {UpdatePassword} = require(path.join(__dirname,'../../src/database_controls/User'))
/*---------------------------------------------------------------*/
...
console.log("Isuerpass",iduser)
    ...
    window_edit_password.loadFile('src/section_main/Edit_password.html');
...
ipcMain.on("sms-alert-system-pass",(event,sms)=>{
    ...
    dialog.showMessageBox(window_edit_password,{
        title: 'Notificación',
          type:'warning',
      message: sms,
      icon: 'info',
